...
export function CourseInfoSidebar({ course }: CourseInfoSidebarProps) {
  ...
  const handleBuyNow = () => {
    // Logique d'achat
    console.log('Achat du cours:', course.id)
  }

  const handleShare = (platform: string) => {
    const url = window.location.href
    const text = `Découvrez ce cours: ${course.title}`
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <div className="mb-2">
          <span className="text-5xl font-bold" style={{ color: '#57C5F7' }}>
            {formatPrice(course.price)}
          </span>
          {course.price && (
            <span className="text-gray-500 text-sm ml-2">(one-time purchase)</span>
          )}
        </div>
        <Button
          variant="primary"
        ...
        {course.students && (
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center justify-between flex-1">
              ...
              <span className="font-semibold text-gray-900">{formatStudents(course.students)}</span>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center justify-between flex-1">
            ...
            <span className="font-semibold text-gray-900">
              {course.certificateIncluded ? 'Yes' : 'No'}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-3 flex-1">
            ...
            <button
              ...
              className="flex-1 h-10 flex items-center justify-center transition-opacity hover:opacity-80"
              style={{ color: '#57C5F7' }}
